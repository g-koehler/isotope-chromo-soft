...
*/	
    ...
    for (i = 0; i < mainarraysize; i++) {
	    ...
	    for (cntr = 0; cntr < (peakend - peakstart) + 100; cntr++) {
	    	peak_array_44[cntr]=mass44[peakstart -50 + cntr];
            	peak_array_45[cntr]=mass45[peakstart -50 + cntr];  /*fill up peak arrays */
            	peak_array_46[cntr] = mass46[peakstart -50 + cntr]; 
	    }
	    i=peakend;
        }   
